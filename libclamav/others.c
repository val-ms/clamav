...
static cl_error_t append_virus(cli_ctx *ctx, const char *virname, IndicatorType type)
{
    ...
    add_successful = evidence_add_indicator(
        ...
        virname,
        type,
        ctx->recursion_stack[ctx->recursion_level].entity_id,
        &add_indicator_error);
    if (!add_successful) {
...
cl_error_t cli_recursion_stack_push(cli_ctx *ctx, cl_fmap_t *map, cli_file_t type, bool is_new_buffer, uint32_t attributes)
{
    ...
    // Assign a unique entity ID to the new container.
    new_container->entity_id = ctx->entity_count;
    ctx->entity_count++;
    ...
    if (SCAN_COLLECT_METADATA) {
        ...
        status = cli_jsonint(ctx->this_layer_metadata_json, "EntityID", new_container->entity_id);
        if (status != CL_SUCCESS) {
            goto done;
        }
        ...
        if (SCAN_COLLECT_METADATA) {
            ...
            if (NULL != contained_indicators) {
                ...
                for (i = 0; i < num_indicators; i++) {
                    ...
                    if (NULL != indicator) {
                        // Create json object containing name, type, depth, and entity_id
                        ...
                        if (NULL == match_obj) {
                            ...
                            json_object_object_add(match_obj, "EntityID", json_object_new_int((int)entity_id));
