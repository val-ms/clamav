...
static cl_error_t scan_common(cl_fmap_t *map, const char *filepath, const char **virname, unsigned long int *scanned, const struct cl_engine *engine, struct cl_scan_options *scanoptions, void *context)
{
    ...
    *virname = NULL;
    ...
    ctx.recursion_stack[ctx.recursion_level].entity_id = ctx.entity_count;
    ctx.entity_count++;
    ...
    if (ctx.options->general & CL_SCAN_GENERAL_COLLECT_METADATA) {
        ...
        status = cli_jsonint(ctx.metadata_json, "EntityID", ctx.recursion_stack[ctx.recursion_level].entity_id);
        if (status != CL_SUCCESS) {
            cli_errmsg("scan_common: error setting EntityID property in metadata.json\n");
            goto done;
        }
    }
        ...
        if (ctx.options->general & CL_SCAN_GENERAL_ALLMATCHES) {
            ...
            for (i = 0; i < num_potentially_unwanted_indicators; i++) {
                const char *pua_alert = evidence_get_indicator(
                    ...
                    NULL  // Don't need to get the entity ID here.
                );
