...
static cl_error_t scanraw(cli_ctx *ctx, cli_file_t type, uint8_t typercg, cli_file_t *dettype)
{
    ...
    if ((typercg) &&
        ...
        (type != CL_TYPE_CABSFX) &&     /* " */
        (type != CL_TYPE_7ZSFX) &&      /* " */
        (type != CL_TYPE_OOXML_WORD) && /* OOXML are ZIP-based and file-type scanning will double-extract their contents. */
                    ...
                    case CL_TYPE_MHTML:
                        if (SCAN_PARSE_MAIL && (DCONF_MAIL & MAIL_CONF_MBOX)) {
                            if ((ctx->recursion_stack[ctx->recursion_level].type >= CL_TYPE_TEXT_ASCII) &&
                                ...
                                (void)cli_recursion_stack_change_type(ctx, fpt->type, true);
                        ...
                        if (SCAN_PARSE_ARCHIVE) {
                            ...
                            // if ((ctx->recursion_stack[ctx->recursion_level].type == CL_TYPE_???) ||  ...))
                            {
                                ...
                                if ((iret == CL_TYPE_GPT) && (DCONF_ARCH & ARCH_CONF_GPT)) {
                                    // Reassign type of current layer based on what we discovered
                                    (void)cli_recursion_stack_change_type(ctx, CL_TYPE_GPT, true);
                                    ...
                                    // Reassign type of current layer based on what we discovered
                                    (void)cli_recursion_stack_change_type(ctx, CL_TYPE_MBR, true);
                ...
                case CL_TYPE_HTML:
                    if (cli_recursion_stack_get_type(ctx, -2) == CL_TYPE_AUTOIT) {
                        ...
                        *dettype = CL_TYPE_HTML;
                        (void)cli_recursion_stack_change_type(ctx, CL_TYPE_HTML, true);
                ...
                case CL_TYPE_MAIL:
                    if (SCAN_PARSE_MAIL && type == CL_TYPE_TEXT_ASCII && (DCONF_MAIL & MAIL_CONF_MBOX)) {
                        *dettype = CL_TYPE_MAIL;
                        (void)cli_recursion_stack_change_type(ctx, CL_TYPE_MAIL, true);
    ...
    char **file_type_out)
{
    ...
    *virname = NULL;
    ...
    if (NULL != hash_alg) {
        ...
        } else if (0 == strcmp(hash_alg, "sha1") || (0 == strcmp(hash_alg, "SHA1"))) {
            requested_hash_type = CLI_HASH_SHA1;
        } else if ((0 == strcmp(hash_alg, "sha2-256")) || (0 == strcmp(hash_alg, "SHA2-256")) ||
                   (0 == strcmp(hash_alg, "sha256")) || (0 == strcmp(hash_alg, "SHA256"))) {
