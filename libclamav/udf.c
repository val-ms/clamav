...
static bool parseFileEntryDescriptor(cli_ctx *ctx, FileEntryDescriptor *fed, PartitionDescriptor *pPartitionDescriptor, LogicalVolumeDescriptor *pLogicalVolumeDescriptor, FileIdentifierDescriptor *fileIdentifierDescriptor)
{
    ...
    allocation_descriptor = (void *)fed + (file_entry_descriptor_size - allocation_descriptor_len);
...
cl_error_t cli_scanudf(cli_ctx *ctx, const size_t offset)
{
    ...
    while (1) {
        ...
        switch (tagId) {
            ...
            default: {
                // TODO REMOVE ==========
                // Note to discuss with Andy...
                //
                // Something feels wrong about doing this in `default:`. Is there a specific value we can look for to be certain we found them all?
                // Or maybe we can check if we reached the end of the volume somehow??
                //
                // TODO REMOVE ==========
