...
pub struct IndicatorMeta {
    ...
    /// Entity ID for the layer this indicator was found in.
    entity_id: usize,
}
...
/// Returns a string that is either static, or allocated when reading the database.
/// So the lifetime of the string is good at least until you reload or unload the databases.
/// The out_depth and out_entity_id parameters are optional, and will be size_t pointers for depth and entity_id.
///
/// No parameters may be NULL
    ...
    match indicator_type {
        IndicatorType::Strong => {
            if let Some(meta) = evidence.strong.values().nth(index) {
                // Set out_depth and out_entity_id, if they are not NULL
                if !out_depth.is_null() {
                    *out_depth = meta.last().unwrap().depth;
                }
                if !out_entity_id.is_null() {
                    *out_entity_id = meta.last().unwrap().entity_id;
                }
